...
* ****`executeTransaction(uint256 _txIndex)`****: Executes an approved transaction. This is the function that actually transfers funds. Only owners can execute this. The function "Verifies if enough confirmations exist", "Verifies contract has enough ETH", "Marks transaction as `Executed`"
